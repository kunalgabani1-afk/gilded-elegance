...
export const Route = createFileRoute("/collections")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse heritage jewellery collections — necklaces, rings, earrings, bangles, diamonds and bridal sets." },
    ],
  }),
    ...
    <div className="pt-32 pb-20">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center pt-8 pb-12">
          <p className="text-[11px] tracking-[0.4em] uppercase text-gold-deep">The Maison Archive</p>
          <h1 className="mt-4 font-serif text-[clamp(2.5rem,6vw,5rem)] leading-[1]">The full collection.</h1>
          <p className="mt-6 max-w-xl mx-auto font-serif italic text-lg text-muted-foreground">
            Every piece below is on display at our Jaipur atelier. Step in to see them up close.
          </p>
        </div>

        {/* Floating filter bar */}
        <div className="glass rounded-full p-3 shadow-card-luxe flex flex-wrap items-center gap-3 mb-10 sticky top-24 z-30">
          ...
          <div className="hidden md:flex items-center gap-2 px-3 border-l border-gold/30">
            <SlidersHorizontal size={14} className="text-gold-deep" />
            <input
              type="range"
              min={50000}
              max={MAX_PRICE}
            ...
            />
            <span className="text-xs font-serif w-24">≤ ₹{(maxPrice / 100000).toFixed(1)}L</span>
          </div>
          <select
            value={sort}
